...
namespace news_apis
{
    public class FinancialDevelopmentAPI : BaseFinanceApi
    {
        ...
        protected override Stock? ParseStockResponse(string response) { return new Stock { Open = 1234, High = 1324, Low = 1234, Close = 1234 }; }

        protected override Task<Financials> GetFinanceResponse(Company supplier)
        {
            return Task.FromResult(new Financials
            {
                Date = DateTime.Today,
                Cik = "CBA",
                Revenue = 1324,
